...
터미널을 열고 내 OS의 한 줄을 붙여넣으면 끝입니다. (처음이라면 아래 [OS별 친절 가이드](#먼저-내-컴퓨터부터-고르세요)부터!)
...
이 한 줄로 **CLI 도구**(git, ripgrep, fzf, bat…), **런타임**(Node·Python·Go·Rust),
**셸/프롬프트**(oh-my-zsh + starship + 자동완성), **컨테이너**(Docker, 선택),
**AI 코딩 에이전트**(Claude Code, gajae-code, codex 등)를 설치하고 — **제대로
깔렸는지 검증**까지 합니다.

> **안심하세요.** 이 키트는 **당신 설정을 함부로 덮어쓰지 않습니다.** 이미 있는 건 건너뛰고,
> 몇 번을 다시 돌려도 안전(멱등)하며, 설치한 걸 **되돌리는 제거 스크립트**도 있습니다.
> 실행 전 계획만 보려면 `--dry-run`. 자세한 건 [설치해도 안전한가요?](#설치해도-안전한가요)

---

## 이 키트가 다른 점

- 🧪 **커밋마다 6개 환경에서 진짜로 설치해봅니다** — macOS · Windows · Ubuntu · Fedora ·
  Arch · openSUSE에서 설치→검증→제거 전 과정을 CI가 자동 반복. **두 번 연속 설치**(멱등성)와
  **이전 버전→최신 업그레이드**까지 테스트합니다. "내 환경에서 되나?"를 문서가 아니라
  테스트가 보증해요.
- 🛟 **설정 파일 이중 안전장치** — `~/.zshrc`/PowerShell 프로필을 처음 고치기 전
  **자동 백업**(`.bak`)을 만들고, 표시된 구역만 편집하며, 마커가 손상된 파일은
  **수정을 거부**합니다.
- 🏢 **회사 PC가 1급 시민** — 관리자 권한(sudo) 없어도 유저 도구는 끝까지 설치,
  Docker Desktop은 라이선스 때문에 기본 미설치, 한국어 프로필 인코딩 보존,
  `--profile work` 프리셋 제공.
- 🔁 **설치하고 끝이 아닙니다** — `--doctor`로 상태 진단, `--update`로 키트 자체 업데이트,
  언제든 재실행 가능. 일회용 스크립트가 아니라 관리되는 도구예요.
- 🔍 **공급망 투명성** — 외부 설치 스크립트 핀·검증, GitHub Actions SHA 고정,
  [SECURITY.md](./SECURITY.md). 뭘 믿고 실행하는지 숨기지 않습니다.
...
- 회사 PC라면 처음부터 가볍게: `.\install.ps1 -Profile work`
...
> **WSL2 + Ubuntu까지 (베타)**: `.\install.ps1 -Only wsl` 하나로 WSL2를 켜고 그 안에
> Linux 킷(Hermes 포함)까지 설치할 수 있어요. 자세한 흐름은
> [windows/README.md](./windows/README.md#wsl2--ubuntu-the-wsl-step) 참고.
...
이 키트는 한 번 쓰고 버리는 스크립트가 아니라, 개발 환경을 계속 관리해주는 도구입니다.
(윈도우는 `--flag` 대신 `-Flag` 형태)

```sh
./install.sh --doctor     # 뭔가 이상할 때: 도구별 ✓설치됨 / ✗누락 / !PATH문제 진단
./install.sh --update     # 키트 최신 버전을 받아온 뒤 그대로 재실행
./install.sh              # 언제든 재실행 — 이미 된 건 건너뛰고 빠진 것만 채움 (멱등)
```

**프리셋으로 골라 깔기:**

```sh
./install.sh --profile minimal   # 도구 + 런타임 + 셸 + git만 (Docker/에이전트 제외)
./install.sh --profile work      # 회사 PC용: Docker 등 무거운 것 제외
./install.sh --only agents       # 특정 단계만 / --skip docker 처럼 빼기도 가능
```
...
- **멱등(idempotent)**: 몇 번을 다시 돌려도 문제 없습니다 — CI가 매 커밋 **두 번 연속
  설치**해서 증명합니다.
...
> 멱등성(2회 연속 설치)과 **이전 릴리스→최신 업그레이드 경로**도 테스트에 포함돼요.
...
```sh
STARTER_KIT_BRANCH=v0.5.0 bash -c "$(curl -fsSL https://raw.githubusercontent.com/Heoooooon/lazy-starter-kit/v0.5.0/install.sh)"
```

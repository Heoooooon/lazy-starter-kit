...
step_brew() {
  ...
  selected | grep -qx runtimes && brewfiles+=("$ROOT/Brewfile.runtimes")
  selected | grep -qx docker && brewfiles+=("$ROOT/Brewfile.docker")
  ...
  if ! have brew; then
    if [[ "$DRY_RUN" == "1" ]]; then
      for brewfile in "${brewfiles[@]}"; do
        info "[dry-run] Homebrew not present yet — would then: brew bundle install --file=$brewfile"
      done
      return 0
    fi
  ...
  for brewfile in "${brewfiles[@]}"; do
    if run brew bundle install --file="$brewfile"; then
      ok "$(basename "$brewfile") applied"
    else
      export KIT_INSTALL_FAILED=1
      warn "some entries in $(basename "$brewfile") failed — continuing"
    fi
  done
}

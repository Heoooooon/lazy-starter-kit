...
if [[ "${1:-}" == __fixture_* ]]; then
  ...
  # Reproduce the hostile inherited environment Homebrew explicitly rejects.
  export CI=1
...
if [[ -n "${CI-}" && -n "${INTERACTIVE-}" ]]; then
  printf 'UPSTREAM_CONFLICT:CI+INTERACTIVE\n' >&2
  exit 90
fi
if [[ -n "${INTERACTIVE-}" && -n "${NONINTERACTIVE-}" ]]; then
  printf 'UPSTREAM_CONFLICT:INTERACTIVE+NONINTERACTIVE\n' >&2
  exit 91
fi
printf 'FAKE_INSTALLER:CI=%s:NONINTERACTIVE=%s:INTERACTIVE=%s\n' \
  "${CI:-}" "${NONINTERACTIVE:-}" "${INTERACTIVE:-}"
...
[[ "$result" == *"FAKE_INSTALLER:CI=1:NONINTERACTIVE=1:INTERACTIVE="* ]] \
  || fail "--yes did not remove inherited INTERACTIVE before launching Homebrew"
[[ "$result" != *"UPSTREAM_CONFLICT:"* ]] \
  || fail "--yes passed a conflicting environment to Homebrew"
printf 'ok   --yes sanitizes Homebrew non-interactive mode\n'
...
[[ "$result" == *"FAKE_INSTALLER:CI=:NONINTERACTIVE=:INTERACTIVE=1"* ]] \
  || fail "interactive mode did not remove inherited CI/NONINTERACTIVE"
[[ "$result" != *"UPSTREAM_CONFLICT:"* ]] \
  || fail "interactive mode passed a conflicting environment to Homebrew"
printf 'ok   a real TTY sanitizes Homebrew interactive mode\n'
